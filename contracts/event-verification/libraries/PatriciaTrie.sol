...
library PatriciaTrie {
    ...
    referenced by hash or nested in the branch node is the total RLP-encoded node is less than 32 bytes memory (nested node),
    ...
    */
    ...
    function processBranchNode(RLP.RLPItem[] memory _currentNode, uint _traversedNibbles, bytes memory _path, bytes memory _value) private returns (bytes32, uint) {
        ...
        uint16 nextPathNibble = uint16(uint8(_path[_traversedNibbles]));
    ...
    function toNibbleArray(bytes memory b, bool hexPrefixed) private pure returns (bytes memory) {
        ...
        for (uint i = 0; i < b.length; i++) {
            ...
            if (hexPrefixed && i == 0) {
                if (nibbles[0] == 0x01 || nibbles[0] == 0x03) {
